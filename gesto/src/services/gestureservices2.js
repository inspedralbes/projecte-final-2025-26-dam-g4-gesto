export class GestureService {
       ...
       return new Promise((resolve, reject) => {
           script.onload = async () => {
               try {
                   ...
                   // --- CARGA DINÁMICA DE MODELO Y CLASES ---
                   try {
   ...
   _predirSigne(ma) {
       ...
       return tf.tidy(() => {
           ...
           // 👁️ CHIVATO EN CONSOLA: Descomenta la siguiente línea si quieres ver los porcentajes
           // console.log(`🤔 La IA ve: ${gestAdivinat} al ${(confianca * 100).toFixed(1)}%`);

           // 🛡️ FILTRO DE SEGURIDAD EXTREMO: Subido al 95% (0.95)
           if (confianca > 0.95) {
               return gestAdivinat;
           }
           
           // Si no llega al 95% de seguridad, devolvemos "none" (ningún gesto claro)
           return "none";
       });
   }
   ...
   _analitzarMoviment(mans, timestamp) {
       ...
       if (mans.length > 0) {
           ...
           // Si la predicción no pasa el filtro del 95%, signeActual será "none"
